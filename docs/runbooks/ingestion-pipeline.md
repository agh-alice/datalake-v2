...
| `SITESONAR_LIMIT` | unset (no limit) | `sitesonar.py`'s `_resolve_limit()` (final-review N5-part) -- caps `run-sitesonar`'s per-run new-file fetch count, same effect as the pre-existing `--limit` CLI flag (which always wins if both are given). **kind-only**, independent-review finding 2026-08-07: `envs/prod/orchestration/values-kind.yaml` sets `ingest.sitesonarLimit: "5"`, which values-gates an 11th `data:` entry onto `templates/external-secrets.yaml`'s `ingest-env` ExternalSecret (`{{- if .Values.ingest.sitesonarLimit }}`); `hack/kind-up.sh` provisions the matching `SITESONAR_LIMIT=5` literal on the remote Secret in `eso-secret-source` for ESO to resolve. Together these mean a scheduled kind tick never attempts the real ~1740-file alimonitor.cern.ch backlog. **Prod deliberately leaves `ingest.sitesonarLimit` empty** (`envs/prod/orchestration/values.yaml`'s default) -- its ExternalSecret never requests this property at all, so production runs the real, unbounded backlog drain, unchanged. |
